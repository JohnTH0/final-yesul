...
public class EmailAsyncService {
    ...
    public void sendSignUpVerificationMail(User user) {
        ...
        log.info("가입 인증 메일 발송: {}", user.getEmail());
    }
    ...
    public void sendPasswordResetMail(User user) {
        ...
        String link = "http://localhost:8080/user/password-reset?"
                + "email=" + user.getEmail()
                + "&token=" + user.getEmailCheckToken();
        ...
        log.info("비밀번호 재설정 메일 발송: {}", user.getEmail());
    }

    /** 내부 공통 메서드 */
    private void sendHtmlMail(String to, String subject, String bodyHtml) {
        try {
